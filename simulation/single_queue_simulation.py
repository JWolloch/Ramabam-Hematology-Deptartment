...
def run_single_queue_simulation(parameter_dict: dict=None):
    if parameter_dict is None:
        model_parameters = ModelParametersSingleQueue()
    else:
        model_parameters = ModelParametersSingleQueue.from_dict(parameter_dict)
    ...
    for epoch in range(simulation_configuration.num_epochs):
        ...
        for patient_list in list_of_patients:
            for index, patient in enumerate(patient_list):
                ...
                if nurse_station == "transplant_nurse_station":
        ...
        while Calendar.N() > 0 and not utils.all_left_department(list_of_patients):
            ...
            if NextEvent.EventType == "q_flow_station_start_of_waiting":
                ...
                counters["general_nurse_station"] += 1
                if counters["general_nurse_station"] == nurse_num_patients["general_nurse_station"]:
                    general_nurse_station_busy_avg.append(general_nurse_station.Mean())  # Use actual busy time
                    general_nurse_station_busy_var.append(general_nurse_station.Variance())  # Use actual busy time variance
                utils.set_patient_blood_test_results_ready_time(NextEvent.WhichObject, Calendar, model_parameters)
                utils.general_nurse_station_service_end(NextEvent.WhichObject, general_nurse_station, general_nurse_station_queue, general_nurse_station_wait_time, Calendar)
        ...
        if epoch == 0:
            if simulation_configuration.personalize_schedule:
                utils.generate_patient_attributes_csv(list_of_patients, "results_directory/same_patient_profiles/single_queue_v2/personalized/patient_attributes.csv")
            else:
                utils.generate_patient_attributes_csv(list_of_patients, "results_directory/same_patient_profiles/single_queue_v2/current_state/patient_attributes.csv")
    ...
    print(f"general_nurse_station_wait_time_avg: {len(general_nurse_station_wait_time_avg)}")
    print(f"general_nurse_station_queue_length_avg: {len(general_nurse_station_queue_length_avg)}")
    print(f"general_nurse_station_busy_avg: {len(general_nurse_station_busy_avg)}")
    print(f"transplant_nurse_station_wait_time_avg: {len(transplant_nurse_station_wait_time_avg)}")
    print(f"transplant_nurse_station_queue_length_avg: {len(transplant_nurse_station_queue_length_avg)}")
    print(f"transplant_nurse_station_busy_avg: {len(transplant_nurse_station_busy_avg)}")
    ...
    print(f"general_nurse_station_scheduled_vs_actual_time_diff_avg: {len(general_nurse_station_scheduled_vs_actual_time_diff_avg)}")
    print(f"transplant_nurse_station_scheduled_vs_actual_time_diff_avg: {len(transplant_nurse_station_scheduled_vs_actual_time_diff_avg)}")
    print("**********************************************************************************")
    ...
    os.makedirs("results_directory/same_patient_profiles/single_queue", exist_ok=True)
    if simulation_configuration.personalize_schedule:
        averages_df.to_csv(f"results_directory/same_patient_profiles/single_queue_v2/personalized/averages_data.csv")
        print(f"Simulation Results Saved to results_directory/same_patient_profiles/single_queue_v2/personalized/averages_data.csv")
    else:
        averages_df.to_csv(f"results_directory/same_patient_profiles/single_queue_v2/current_state/averages_data.csv")
        print(f"Simulation Results Saved to results_directory/same_patient_profiles/single_queue_v2/current_state/averages_data.csv")
    print("**********************************************************************************")
    ...
    os.makedirs("results_directory/same_patient_profiles/single_queue", exist_ok=True)
    if simulation_configuration.personalize_schedule:
        variances_df.to_csv(f"results_directory/same_patient_profiles/single_queue_v2/personalized/variances_data.csv")
        print("Simulation Variance Results Saved to results_directory/same_patient_profiles/single_queue_v2/personalized/variances_data.csv")
    else:
        variances_df.to_csv(f"results_directory/same_patient_profiles/single_queue_v2/current_state/variances_data.csv")
        print("Simulation Variance Results Saved to results_directory/same_patient_profiles/single_queue_v2/current_state/variances_data.csv")
    print("**********************************************************************************")
...
# Only run the simulation if this script is executed directly
if __name__ == "__main__":
    run_single_queue_simulation()