...
class CTStat:
    '''
    Class of objects for continuous-time statistics. The statistics are computed over time.
...
class DTStat():
    '''
    Class of objects for discrete-time statistics. The statistics are computed only over the observations (without time).
...
class EventCalendar:
    ...
    def binary_Insert(self, addedEvent):
        i = self.find_index(addedEvent.EventTime)
        self.ThisCalendar.insert(i, addedEvent)
    ...
    def find_index(self, t):
        ...
        while start <= end:
            mid = (start + end) // 2
            tMid = self.ThisCalendar[mid].EventTime
            if tMid == t:
                return mid
            elif tMid < t:
                start = mid + 1
...
class FIFOQueue:
    ...
    def Abandon(self, X):
        '''
        Removes and updates queue statistics
        '''

        if len(self.ThisQueue) > 0:
            self.ThisQueue.remove(X)
            self.WIP.Record(float(self.NumQueue()))
